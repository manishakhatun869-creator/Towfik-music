...
package com.towfik.music.lrclib

import com.towfik.music.lrclib.models.Track
import com.towfik.music.lrclib.models.bestMatchingFor
...
object LrcLib {
    ...
    suspend fun getLyrics(
        ...
        val res = when {
            duration == -1 -> {
                tracks.bestMatchingFor(duration, title, artist)?.syncedLyrics?.let { LrcLib.Lyrics(it) }
            }
            else -> {
                tracks.bestMatchingFor(duration)?.syncedLyrics?.let { LrcLib.Lyrics(it) }
            }
        }
